...
def model_forward_with_regression_loss(
    model: RewardModel,
    input_ids: torch.Tensor,
    labels: torch.Tensor,
    weights: torch.Tensor,
    ...
    with torch.autocast(device_type="cuda", enabled=False):
        dtype_logits = logits.float()
        dtype_logits = torch.sigmoid(dtype_logits)
        loss = (dtype_logits.view(-1).float() - labels.view(-1).float()) ** 2
        weights = weights.view(-1).float()
        loss = (loss * weights).sum() / (weights.sum() + 1e-6)
                    ...
                    if ex_idx + micro_train_batch_size < input_ids.size(0):
                        with torch.cuda.amp.autocast(dtype=args.compute_dtype):
                            if args.regression == True:
                                loss = model_forward_with_regression_loss(
                                    model,
